...
def resize_images(input_dir, output_dir, scale=0.5):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.png'):
            ...
            img = (np.array(img) / 255.).astype(np.float32)  # 归一化到0-1之间
            H, W = img.shape[:2]
            new_H, new_W = int(H * scale), int(W * scale)
            resized_img = cv2.resize(img, (new_W, new_H), interpolation=cv2.INTER_AREA)  # 使用INTER_AREA插值方法缩小图片
            output_path = os.path.join(output_dir, filename)
            imageio.imwrite(output_path, (resized_img * 255).astype(np.uint8))  # 保存缩小后的图片，恢复到0-255范围
